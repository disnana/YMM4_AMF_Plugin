...
- Windows 11（x64）
- [YMM4 / YMM4 Lite](https://manjubox.net/ymm4/)の.NET 10対応版で、`IVideoFileWriter3`を持つ版が必要です。v0.2.0のAPI・独立テストと実プロジェクトログはYMM4 v4.56.1.1で確認しています。古いAPIのYMM4ではGPU直渡しをOFFにしてもプラグインを読み込めません。
...
| GPU直渡し（実験・IVideoFileWriter3） | オフ | 入力経路の比較オプション。YMM4の描画済みGPUフレームを直接受け取り、CPU読み取り用ビットマップへの中間コピーを省きます。AMFの所有テクスチャへのGPUコピーは残ります。 |
| デバッグログを書き出す | オフ | エラー調査時にオンにしてから出力します。保存先は後述。 |
| プロファイリング結果を書き出す（集計JSON） | オフ | 通常出力・破棄モードの受け取り時間や処理区間を集計し、終了時に`出力名.amf_profile.json`へ保存します。デバッグログとは独立しています。 |
| 計測専用：映像・音声を破棄（MP4を作成しない） | オフ | YMM4から届いた映像・音声を、コピー・エンコード・音声保持・動画保存せずに返します。診断用で、視聴できる動画は生成されません。 |
...
GPU直渡しはこの動画出力設定で操作します。別のツールやHarmonyは不要です。チェックは次に開始する出力へ適用され、進行中の出力経路は変わりません。OFFでもAMFハードウェアエンコードであり、CPUエンコードに切り替える機能ではありません。従来経路も、プラグインではCPUに画素を読み出しません。

実プロジェクトのログでGPU直渡しON/OFF両方の7,259フレーム完了を確認しましたが、この1往復では高速化を確認できていません。長時間安定性も未確認です。まず短い区間を別名で出力して確認してください。[v0.2.0の変更と検証範囲](https://github.com/disnana/YMM4_AMF_Plugin/blob/master/docs/release-notes/v0.2.0.md)を参照できます。

設定のファイル保存は未実装のため、YMM4を再起動した後は出力設定を確認してください。

## ボトルネックを調べる

以下の診断オプションはv0.2.0で追加した機能です。v0.1.1パッケージには含まれません。通常の動画制作では既定のOFFのまま利用できます。

同じプロジェクト・同じ範囲で、まず「プロファイリング結果を書き出す」だけをオンにして通常出力し、次に「計測専用：映像・音声を破棄」もオンにして比較します。両方ともデバッグログはオフにし、必ず別々の新しい出力名を指定してください。破棄モードではMP4を作りませんが、プロファイリングがオンならJSONは保存します。設定したMP4が既に存在していても、本プラグインの破棄モードはそれを更新しません。

JSONは動画の出力先フォルダーへ保存します（例：`通常.mp4.amf_profile.json`と`破棄.mp4.amf_profile.json`）。同名のJSONは最新結果で置き換えます。破棄モードでプロファイリングもオフにした場合、プラグインからは動画・計測JSONのどちらも生成しません。調査が終わったら両方のチェックを外してください。

両DLLを同じビルドへ更新する必要があります。区間の意味、比較の手順、結果の読み方は[プロファイリングガイド](https://github.com/disnana/YMM4_AMF_Plugin/blob/master/docs/profiling.md)にまとめています。破棄モードとの差は、GPU競合や待ち時間の変化も含むため、そのままエンコード時間とは解釈できません。
